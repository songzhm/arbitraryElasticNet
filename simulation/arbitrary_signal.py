...
for i in q[0:T]:
    x[i] = np.sign(np.random.randn(1)) * np.random.rand(1)
...
upbo = 1 * np.ones(N)
for i in q[0:T]:
    lowbo[i] = np.minimum(x[i], 0)
    upbo[i] = np.maximum(x[i], 0)
coeffs = s.solve(A, y, lam_1, lam_2, lowbo, upbo, dvec, Pmat, err_tol=1e-8, text='On', text_fr=10000)
