...
log('poster_experiments_argen.txt',
...
parameters = {'lam_2': lam_2_pool,
              'sigma_ds': np.random.RandomState(2).randint(1,11,size=(5,p))/10}
for j in [0,1]:
    ...
    for i in range(nps):
        ...
        if j == 0:
            ...
            base_reg = GeneralizedElasticNetRegressor(beta=beta, tune_lam_1=True, target_number=portfolio_size)
            clf = RandomizedSearchCV(base_reg, parameters, cv=3, scoring=mse_score, verbose=4, n_jobs=-1)
    ...
    log('poster_experiments_argen.txt', '{set0}, {set2}, {set3}, {set4}, {set5}, {num0:.2%},{num1:.2%},{num2:.2%},'
        '{num3:.3%},{num4:.3%},{num5:.2%},{num6: .2e}'
        .format(set0=update_frequency,
                set2=portfolio_size,
                set3=training_month,
...
np.save('ARGEN_sigma_returns', results_return)
np.save('ARGEN_sigma_coefs', results_coefs)
np.save('ARGEN_sigma_dates', dates)
